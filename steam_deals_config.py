...
def load_user_config(config_file: Path = CONFIG_FILE, *, load_json_file=_default_load_json_file) -> dict:
    return load_json_file(config_file, {})


def save_user_config(config_file: Path, cfg: dict, *, write_json_file=_default_write_json_file) -> None:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--interactive", action="store_true", help="Habilita prompts en terminal para configurar valores faltantes")
    ...
    parser.add_argument("--genre", nargs="*", metavar="GENRE", help="Géneros de interés (ej. --genre roguelike --genre indie)")
    parser.add_argument("--no-cache", action="store_true", help="Re-fetch aunque haya caché válida")
    parser.add_argument("--family-json", nargs="?", const="", default=None, help="Ruta al JSON de biblioteca familiar. Sin valor = omitir")
    ...
    parser.add_argument("--watchlist", nargs="*", metavar="CMD", help="Watchlist: add APPID PRECIO / remove APPID / list")
    parser.add_argument("--budget", type=float, metavar="MXN", help="Presupuesto en MXN — recomendación óptima de compras")
    parser.add_argument("--compare", metavar="VANITY2", help="Comparar tu wishlist con otro perfil de Steam")
    ...
    parser.add_argument("--schedule", type=float, metavar="HOURS", help="Ejecutar automáticamente cada N horas (ej: --schedule 6)")
    return parser
...
def _can_prompt(args, stdin, script_path: Path) -> bool:
    ...
    return bool(not args.web_run and is_tty and (args.interactive or not _has_local_cache(script_path)))
...
def _from_arg_cfg_or_ask(arg_val, cfg: dict, cfg_key: str, prompt: str, default, *, can_prompt: bool, interactive_keys: list[str], input_fn) -> str:
...
def _resolve_hltb(args, cfg: dict, *, can_prompt: bool, interactive_keys: list[str], input_fn):
    if args.hltb:
    ...
    raw = _ask(input_fn, "Ruta al CSV de HLTB (Enter para omitir)", can_prompt=can_prompt)
    return Path(raw).expanduser() if raw else None


def _resolve_output_dir(args, cfg: dict, script_path: Path, *, can_prompt: bool, interactive_keys: list[str], input_fn) -> Path:
    if args.output:
    ...
    raw = _ask(input_fn, "Directorio de salida para los MD", script_dir, can_prompt=can_prompt)
    return Path(raw or script_dir).expanduser()


def _resolve_discount(args, cfg: dict, *, can_prompt: bool, interactive_keys: list[str], input_fn) -> int:
...
def _resolve_genres(args, cfg: dict, *, can_prompt: bool, interactive_keys: list[str], input_fn) -> list[str]:
    ...
    raw = _ask(input_fn, "Géneros de interés (coma-separados, Enter para omitir)", can_prompt=can_prompt)
...
def get_config(
    *,
    ...
    vanity = _from_arg_cfg_or_ask(args.vanity, cfg, "vanity", "Vanity URL, Steam ID, o link de perfil", "BG00G", can_prompt=can_prompt, interactive_keys=interactive_keys, input_fn=input_fn) or "BG00G"
    hltb = _resolve_hltb(args, cfg, can_prompt=can_prompt, interactive_keys=interactive_keys, input_fn=input_fn)
    output_dir = _resolve_output_dir(args, cfg, script_path, can_prompt=can_prompt, interactive_keys=interactive_keys, input_fn=input_fn)
    discount = _resolve_discount(args, cfg, can_prompt=can_prompt, interactive_keys=interactive_keys, input_fn=input_fn)
    genres = _resolve_genres(args, cfg, can_prompt=can_prompt, interactive_keys=interactive_keys, input_fn=input_fn)
    ...
    return args.web_run, args.interactive, key, vanity, hltb, output_dir, discount, genres, no_cache, family_json, itad_key, _build_filters(args, cfg)
