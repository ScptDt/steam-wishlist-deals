...
def _render_deal_row(deal: dict, reviews: dict[str, dict], deck_compat: dict[str, int]) -> str:
    appid = deal["appid"]
...
def generate_share_html(deals, vanity, min_discount, sale_name="", top_picks=None, reviews=None, deck_compat=None):
    ...
    picks_html = ""
    if top_picks:
        pick_cards = "".join(_render_top_pick_card(top_pick) for top_pick in top_picks[:5])
        picks_html = (
