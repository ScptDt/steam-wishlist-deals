...
Corte docs-first OAuth-first ITAD cerrado 2026-06-18:

- Decisión: sin una muestra `/deals/v2` redactada, no se implementa más código ni se afirma cobertura real. El próximo corte técnico debe partir de una muestra local redactada o de un live smoke explícitamente aprobado.
- UX futura propuesta: un modo “Conectar ITAD con OAuth” puede alimentar una caché local de deals `/deals/v2` como contexto amplio de ofertas; debe mostrarse como cobertura parcial/paginada, no como refresh exacto por wishlist.
- Fallback avanzado: la API key se mantiene separada para lookup/prices exactos por Steam AppID/UUID hasta que ITAD confirme Bearer para `/games/lookup/v1` y `/games/prices/v3`, o hasta que se diseñe otro backend/fuente con aprobación aparte.
- Contrato de caché futuro: guardar solo payloads/redacciones necesarios para `external_offers` (`list`/`items`, tienda, precio, moneda, DRM, URL segura, timestamps y mapping ITAD→Steam si existe); nunca guardar tokens, refresh tokens, client secret, API key, callback codes, cookies, headers auth ni datos de cuenta.
- Recolección de muestra redactada: pegar/guardar únicamente el body JSON de `/deals/v2`; reemplazar o eliminar `access_token`, `refresh_token`, `client_secret`, `ITAD-API-Key`, cookies, `Authorization`, callbacks con `code`, username/email y rutas locales. Si hay duda, redactar el campo completo antes de compartirlo.
- Criterio para avanzar a código: una muestra fixture demuestra shape/campos suficientes y se define si el objetivo es solo diagnóstico de cobertura, caché local OAuth-first o UI/CLI opt-in. Validación mínima: tests fixture-only + `git diff --check`; red real sigue fuera salvo aprobación explícita.
