...
from steam_deals_behavioral import (
    ...
    load_player_manual_preferences as module_load_player_manual_preferences,
...
from steam_deals_generator import (
    ...
    load_player_manual_preferences as generator_load_player_manual_preferences,
...
class PersonalizedRecommendationsTests(unittest.TestCase):
    ...
    def test_build_taste_priority_contract_tolerates_missing_genre_distribution(self) -> None:
        contract = module_build_taste_priority_contract(
            deals=[{"appid": "10", "name": "Action Deal", "score": 70, "discount": 60, "genres": ["Action"]}],
            personalized_recommendations={
                "items": [
                ...
                "profile": {"library_summary": {"genre_distribution": None}},
            },
        )
...
class BehavioralSignalsTests(unittest.TestCase):
    ...
    def test_build_player_behavior_profile_partial_from_wishlist_terms_only(self) -> None:
        ...
        self.assertIn("local_activity_unavailable", payload["reasons"])
        self.assertIn("library_summary_unavailable", payload["reasons"])
    ...
    def test_build_player_behavior_profile_uses_personalized_profile_defensively(self) -> None:
        payload = module_build_player_behavior_profile(
            personalized_recommendations={
                "profile": {
                    "activity_terms": [{"term": "Co-op", "weight": 4}],
                    "library_summary": {
                        "top_terms": [{"term": "Horror", "weight": 2}],
                        "genre_distribution": None,
                        "local_path": "/home/example-user/SteamTools/private.json",
                    },
                },
                "raw_debug": {"playtime_2weeks": 999, "appid": "1966720"},
            },
        )
        ...
        self.assertEqual(payload["status"], "partial")
        self.assertEqual(payload["source_signals"], ["personalized_recommendations.profile"])
        self.assertIn("manual_preferences_missing", payload["reasons"])
        self.assertTrue(any(item["id"] == "coop_teamwork" for item in payload["preferred_families"]))
        self.assertTrue(any(item["id"] == "horror_tension" for item in payload["preferred_families"]))
        self.assertNotIn("/home/example-user", dumped)
        self.assertNotIn("playtime_2weeks", dumped)
        self.assertNotIn("1966720", dumped)
    ...
    def test_normalize_player_manual_preferences_accepts_wrapper_and_allowed_fields(self) -> None:
        payload = module_normalize_player_manual_preferences(
            {
                "manual_preferences": {
                    "families": ["coop_teamwork"],
                    "behavioral_loops": ["shared_objective_pressure"],
                    "descriptors": ["short_session"],
                    "terms": ["Co-op"],
                    "favorite_games": [
                        {
                            "appid": "1966720",
        ...
        self.assertEqual(payload["preferred_families"], ["coop_teamwork"])
        self.assertEqual(payload["preferred_loops"], ["shared_objective_pressure"])
        self.assertEqual(payload["preferred_descriptors"], ["short_session"])
        self.assertEqual(payload["preferred_terms"], ["Co-op"])
        self.assertEqual(payload["favorite_games"], [{"tags": ["Horror", "Online Co-op"]}])
        self.assertNotIn("1966720", dumped)
        self.assertNotIn("/home/example-user", dumped)
        self.assertNotIn("SECRET", dumped)
        self.assertNotIn("playtime_2weeks", dumped)

    def test_load_player_manual_preferences_handles_empty_and_invalid_json(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(module_load_player_manual_preferences(empty_path), {})
            self.assertEqual(generator_load_player_manual_preferences(empty_path), {})
            with self.assertRaises(ValueError):
                module_load_player_manual_preferences(invalid_path)

    def test_load_player_manual_preferences_rejects_unsupported_top_level_shape(self) -> None:
        with TemporaryDirectory() as temp_dir:
            json_path = Path(temp_dir) / "preferences.json"
            json_path.write_text(json.dumps([{"tags": ["Co-op"]}]), encoding="utf-8")

            with self.assertRaises(ValueError):
                module_load_player_manual_preferences(json_path)

    def test_build_player_behavior_fit_matches_profile_to_game_signals(self) -> None:
        ...
        payload = module_build_player_behavior_fit(profile, signals)
        item = payload["items"][0]
        dumped = json.dumps(payload)

        self.assertEqual(payload["schema"], "player_behavior_fit_v1")
        self.assertEqual(payload["source_schemas"], ["player_behavior_profile_v1", "behavioral_signals_v1"])
        self.assertEqual(payload["status"], "available")
        self.assertTrue(payload["advisory_only"])
        self.assertEqual(payload["ranking_impact"], "none")
        self.assertEqual(payload["summary"]["items_count"], 1)
        ...
        self.assertTrue(any(record["id"] == "coop_teamwork" for record in item["matched_families"]))
        self.assertTrue(any(record["id"] == "shared_objective_pressure" for record in item["matched_loops"]))
        self.assertIn("profile_family_match", item["reason_codes"])
        self.assertNotIn("playtime_2weeks", dumped)
        self.assertNotIn("player_behavior_profile", item)

    def test_build_player_behavior_fit_degrades_without_matches_or_sources(self) -> None:
        profile = module_build_player_behavior_profile(
            manual_preferences={"preferred_families": ["horror_tension"]}
        )
        signals = module_build_behavioral_signals(
            [{"appid": "10", "name": "Portal 2", "tags": ["Puzzle"]}]
        )
        no_match = module_build_player_behavior_fit(profile, signals)
        missing_profile = module_build_player_behavior_fit({}, signals)
...
class ConfigTests(unittest.TestCase):
    ...
    def test_get_config_returns_expected_tuple_from_args(self) -> None:
        ...
        self.assertEqual(result[11]["player_preferences_json"], Path("/tmp/player_preferences.json"))
...
class StopApiContractTests(unittest.TestCase):
    ...
    def test_generate_json_sanitizes_player_behavior_profile_payload(self) -> None:
        payload = generate_json(
            deals=[],
            backlog_on_sale=[],
            have_on_sale=[],
            ...
            player_behavior_profile={
                ...
                "confidence": "medium",
                "reason": "/home/example-user/private-reason",
                "reasons": ["manual_preferences_missing", "/home/example-user/private-reason"],
                "source_signals": ["wishlist_terms", "/home/example-user/private-source"],
                "summary": {
                    "families_count": 99,
                    "loops_count": 99,
                    "descriptors_count": 99,
                    "local_path": "/home/example-user/private-summary.json",
                ...
                "preferred_descriptors": [],
                "evidence_summary": {
                    "manual_preferences_count": 1,
                    "raw_playtime": [{"appid": "1966720", "playtime_2weeks": 180}],
                    "local_path": "/home/example-user/private-evidence.json",
                },
                "limitations": ["local_snapshot", "/home/example-user/private-limitation"],
                "raw_playtime": [{"appid": "1966720", "playtime_2weeks": 180}],
                "local_path": "/home/example-user/private-profile.json",
            },
        )

        data = json.loads(payload)
        profile = data["player_behavior_profile"]
        dumped = json.dumps(profile)

        self.assertTrue(profile["advisory_only"])
        self.assertEqual(profile["ranking_impact"], "none")
        self.assertEqual(profile["source_signals"], ["wishlist_terms"])
        self.assertEqual(profile["reasons"], ["manual_preferences_missing"])
        self.assertEqual(profile["limitations"], ["local_snapshot"])
        self.assertEqual(profile["summary"]["families_count"], 1)
        self.assertEqual(profile["evidence_summary"]["manual_preferences_count"], 1)
        self.assertNotIn("/home/example-user", dumped)
        self.assertNotIn("raw_playtime", dumped)
        self.assertNotIn("playtime_2weeks", dumped)
        self.assertNotIn("1966720", dumped)

    def test_generate_json_uses_manual_player_preferences_opt_in(self) -> None:
        payload = generate_json(
            deals=[],
            backlog_on_sale=[],
            have_on_sale=[],
            ...
            player_manual_preferences={
                "preferred_families": ["coop_teamwork"],
                "preferred_terms": ["Co-op"],
                "favorite_games": [{"tags": ["Horror"], "appid": "1966720"}],
            },
        )
        ...
        self.assertEqual(profile["source_signals"], ["manual_preferences"])
        self.assertEqual(data["summary"]["player_behavior_profile_status"], "available")
        self.assertTrue(any(item["id"] == "coop_teamwork" for item in profile["preferred_families"]))
        self.assertNotIn("1966720", json.dumps(profile))

    def test_generate_json_serializes_player_behavior_fit_without_ranking_changes(self) -> None:
        deals = [
            {
                "appid": "1966720",
                "name": "Lethal Company",
        ...
        payload = generate_json(
            ...
            min_discount=0,
            genres=[],
            tags_data={"1966720": {"tags": {"Co-op": 500, "Online Co-op": 450}}},
            player_manual_preferences={
                "preferred_families": ["coop_teamwork"],
                "preferred_loops": ["shared_objective_pressure"],
            },
    ...
    def test_generate_json_omits_invalid_player_behavior_fit_payload(self) -> None:
        ...
        self.assertEqual(data["summary"]["player_behavior_fit_count"], 0)
        self.assertNotIn("player_behavior_fit", data)

    def test_generate_json_sanitizes_player_behavior_fit_payload(self) -> None:
        payload = generate_json(
            deals=[],
            backlog_on_sale=[],
            have_on_sale=[],
                ...
                "items": [
                    {
                        ...
                        "matched_loops": [],
                        "matched_descriptors": [],
                        "reason_codes": ["profile_family_match", "/home/example-user/private-code"],
                        "raw_playtime": [{"appid": "1966720", "playtime_2weeks": 180}],
                    }
                ],
                "limitations": ["local_snapshot", "/home/example-user/private-limitation"],
        ...
        fit = json.loads(payload)["player_behavior_fit"]
        dumped = json.dumps(fit)
    ...
    def test_generated_reports_render_smart_alert_digest_preview(self) -> None:
        ...
        self.assertIn("+15%", md)
        self.assertIn("[Half-Life 2](https://store.steampowered.com/app/20/)", md)
        self.assertIn("-17%", md)
        self.assertNotIn("+-17%", md)
        self.assertIn("[Zero Change](https://store.steampowered.com/app/30/)", md)
        self.assertIn("0%", md)
...
class SmartAlertsTests(unittest.TestCase):
    ...
    def test_smart_alert_channel_readiness_allows_low_volume_opt_in_without_sending(self) -> None:
        digest = module_build_smart_alert_digest(
            deals=[{"appid": "10", "name": "Alpha", "price_raw": 1000}],
            historical_lows={},
            active_bundles={},
        ...
        readiness = module_decide_smart_alert_channel_readiness(
            digest,
        ...
        self.assertTrue(readiness["channel_ready"])
        self.assertEqual(readiness["status"], "ready_for_future_channel_slice")
        self.assertEqual(readiness["requested_channels"], ["telegram"])
        self.assertEqual(readiness["channels"], [])
        self.assertFalse(readiness["send_ready"])
        self.assertFalse(readiness["external_send_enabled"])
        self.assertEqual(readiness["blockers"], [])
        self.assertIn("preview_only_no_external_send", readiness["reason_codes"])
        self.assertIn("low_volume_digest", readiness["reason_codes"])

    def test_smart_alert_channel_readiness_handles_medium_volume_hidden_items(self) -> None:
        digest = module_build_smart_alert_digest(
            deals=[
        ...
        readiness = module_decide_smart_alert_channel_readiness(
            digest,
            requested_channels=["discord", "discord", "email"],
            user_opt_in=True,
            digest_reviewed=True,
        )

        self.assertTrue(readiness["channel_ready"])
        self.assertEqual(readiness["requested_channels"], ["discord"])
        self.assertEqual(readiness["unsupported_channels_count"], 1)
        self.assertEqual(readiness["anti_spam"]["volume_level"], "medium")
        self.assertEqual(readiness["anti_spam"]["total_hidden_count"], 1)
        self.assertIn("medium_volume_digest", readiness["reason_codes"])
        self.assertIn("hidden_items_must_be_disclosed", readiness["reason_codes"])

    def test_smart_alert_channel_readiness_requires_high_volume_approval(self) -> None:
        digest = module_build_smart_alert_digest(
            deals=[
                {"appid": str(index), "name": f"Game {index}", "price_raw": 1000}
                for index in range(1, 13)
        ...
        approved = module_decide_smart_alert_channel_readiness(
            digest,
            requested_channels=["telegram"],
            user_opt_in=True,
            digest_reviewed=True,
        ...
        self.assertIn("high_volume_requires_explicit_approval", readiness["blockers"])
        self.assertTrue(approved["channel_ready"])
        self.assertIn("high_volume_digest", approved["reason_codes"])
        self.assertIn("hidden_items_must_be_disclosed", approved["reason_codes"])
        self.assertFalse(approved["send_ready"])

    def test_smart_alert_channel_readiness_blocks_empty_malformed_or_unreviewed(self) -> None:
        invalid = module_decide_smart_alert_channel_readiness(
            None,
            requested_channels=["telegram"],
            user_opt_in=True,
            digest_reviewed=True,
        ...
        unreviewed = module_decide_smart_alert_channel_readiness(
            empty_digest,
            requested_channels=[],
            user_opt_in=False,
            digest_reviewed=False,
        ...
        self.assertIn("invalid_digest", invalid["blockers"])
        self.assertFalse(unreviewed["channel_ready"])
        self.assertIn("no_alerts_to_send", unreviewed["blockers"])
        self.assertIn("channel_opt_in_required", unreviewed["blockers"])
        self.assertIn("user_channel_opt_in_required", unreviewed["blockers"])
        self.assertIn("digest_review_required", unreviewed["blockers"])

    def test_smart_alert_channel_readiness_forbids_per_game_notifications(self) -> None:
        digest = module_build_smart_alert_digest(
            deals=[{"appid": "10", "name": "Alpha", "price_raw": 1000}],
            historical_lows={},
            active_bundles={},
        ...
        digest = {
            **digest,
            "anti_spam": {**digest["anti_spam"], "per_game_notifications": True},
        }
    ...
    def test_smart_alert_mock_channel_integration_runs_fake_path_only(self) -> None:
        digest = module_build_smart_alert_digest(
            deals=[{"appid": "10", "name": "Alpha", "price_raw": 1000}],
            historical_lows={},
            active_bundles={},
        ...
        self.assertEqual(result["schema"], "smart_alert_mock_channel_integration_v1")
        self.assertEqual(result["status"], "fake_delivery_completed")
        self.assertEqual(result["transport"], "fake")
        self.assertTrue(result["preview_only"])
        self.assertTrue(result["dry_run"])
        ...
        self.assertEqual(result["opt_in_preview"]["schema"], "smart_alert_channel_opt_in_preview_v1")
        self.assertEqual(result["fake_delivery_plan"]["status"], "ready_for_fake_delivery")
        self.assertEqual(result["fake_delivery_result"]["status"], "fake_delivery_completed")
        self.assertEqual(len(captured_payloads), 1)
        self.assertEqual(captured_payloads[0]["channel"], "telegram")
        self.assertEqual(captured_payloads[0]["transport"], "fake")
        self.assertTrue(captured_payloads[0]["preview_only"])
    ...
    def test_smart_alert_mock_channel_integration_blocks_without_fake_sender(self) -> None:
        ...
        self.assertEqual(result["status"], "blocked")
        self.assertIn("fake_sender_required", result["blockers"])
        self.assertEqual(result["fake_delivery_result"]["attempts"], [])
        self.assertFalse(result["fake_send_performed"])
        self.assertFalse(result["send_performed"])
        self.assertFalse(result["send_ready"])
        self.assertFalse(result["external_send_enabled"])
    ...
    def test_smart_alert_mock_channel_integration_blocks_unapproved_high_volume(self) -> None:
        digest = module_build_smart_alert_digest(
            deals=[
                {"appid": str(index), "name": f"Game {index}", "price_raw": 1000}
                for index in range(1, 13)
        ...
        self.assertEqual(result["status"], "blocked")
        self.assertIn("high_volume_requires_explicit_approval", result["blockers"])
        self.assertEqual(result["anti_spam"]["volume_level"], "high")
        self.assertEqual(result["fake_delivery_result"]["attempts"], [])
        self.assertEqual(calls, [])
        self.assertFalse(result["fake_send_performed"])
        self.assertFalse(result["send_performed"])
    ...
    def test_smart_alert_mock_channel_integration_blocks_sensitive_digest(self) -> None:
        digest = module_build_smart_alert_digest(
            deals=[{"appid": "10", "name": "Alpha", "price_raw": 1000}],
            historical_lows={},
            active_bundles={},
        ...
        digest = {**digest, "telegram_token": "blocked-value"}
        calls: list[dict] = []
        ...
        self.assertEqual(result["status"], "blocked")
        self.assertIn("sensitive_channel_credentials_forbidden", result["blockers"])
        self.assertIsNone(result["opt_in_preview"])
        self.assertIsNone(result["fake_delivery_plan"])
        self.assertEqual(result["fake_delivery_result"]["attempts"], [])
        self.assertEqual(calls, [])
        self.assertFalse(result["send_performed"])
        self.assertFalse(result["external_send_enabled"])
...
class RankTopPicksTests(unittest.TestCase):
    ...
    def test_generate_md_renders_personalized_behavioral_explanation(self) -> None:
        md = generate_md(
            deals=[],
            backlog_on_sale=[],
            have_on_sale=[],
        ...
        self.assertIn("## 🎯 Recomendaciones personalizadas", md)
        self.assertIn("- 1. Deep Rock Galactic", md)
        self.assertIn("  - **Por qué podría gustarte:** Social + Co-op / teamwork", md)
        self.assertIn("    - Patrones principales: Social + Co-op / teamwork", md)
        self.assertIn("    - Loops detectados: High-execution co-op", md)
        self.assertIn("    - Señales: Mission based · Short session · Shared objective pressure", md)
        self.assertIn("    - _Señal advisory: no cambia score ni ranking._", md)
        self.assertNotIn("Esta tercera razón no se renderiza", md)
        self.assertNotIn("Fourth cue hidden", md)

    def test_generate_md_omits_personalized_behavioral_explanation_mismatch(self) -> None:
        md = generate_md(
            deals=[],
            backlog_on_sale=[],
            have_on_sale=[],
    ...
    def test_generate_md_omits_invalid_personalized_behavioral_explanation(self) -> None:
        md = generate_md(
            deals=[],
            backlog_on_sale=[],
            have_on_sale=[],
    ...
    def test_generate_md_surfaces_external_offers_risk_gated_section(self) -> None:
        ...
        self.assertIn("no abre carrito ni checkout", md)
        ...
        self.assertIn("| Juego | Tienda | Precio | Estado | Notas | Acción |", md)
        ...
        self.assertIn("Mejor fuera de Steam · Tienda autorizada", md)
        self.assertIn("[Ver tienda (sin carrito)](https://deals.example/hades)", md)
    ...
    def test_generate_html_renders_local_selection_review_tool(self) -> None:
        ...
        self.assertIn("selectionReviewConfidence", html)
        self.assertIn("selectionReviewNextStep", html)
        self.assertIn("selectionReviewWhyGroups", html)
        self.assertIn("selectionReviewConfidenceLabel", html)
        self.assertIn("renderSelectionReviewWhyGroup", html)
        self.assertIn("selection-review-result-confidence", html)
        self.assertIn("selection-review-result-next-step", html)
        self.assertIn("selection-review-result-why", html)
        self.assertIn("Confianza:", html)
        self.assertIn("Siguiente paso:", html)
        self.assertIn("A favor", html)
        self.assertIn("Cuidado", html)
        self.assertIn("Contexto", html)
        self.assertIn("Evaluación local lista: ${review.summary.total_items} juego(s).", html)
    ...
    def test_generate_html_discount_slider_accepts_lower_report_minimum(self) -> None:
        html = generate_html(
            deals=[],
            backlog_on_sale=[],
            have_on_sale=[],
        ...
        self.assertIn('data-default-discount="30"', html)
    ...
    def test_generate_html_surfaces_external_offers_risk_gated_section(self) -> None:
        ...
        self.assertIn("Solo tiendas oficiales/autorizadas · sin checkout", html)
        self.assertIn("Comparativa informativa", html)
        self.assertIn("no compra, no abre carrito", html)
        self.assertIn("no abre carrito ni checkout", html)
        self.assertIn("no abre carrito/checkout", html)
        ...
        self.assertIn("external-offer-chips", html)
        self.assertIn("external-offer-chip", html)
        self.assertIn("Mejor fuera de Steam", html)
        self.assertIn("Tienda autorizada", html)
        self.assertIn("Revisar DRM/región", html)
    ...
    def test_generate_share_html_renders_personalized_behavioral_explanation(self) -> None:
        html = generate_share_html(
            deals=[],
            vanity="gaben",
            min_discount=50,
        ...
        self.assertIn("data-personalized-recommendations-section", html)
        self.assertIn('class="personalized-behavioral-note"', html)
        self.assertIn("Por qué podría gustarte", html)
        self.assertIn("Social + Co-op / teamwork", html)
        self.assertIn("Patrones principales: Social + Co-op / teamwork", html)
        self.assertIn("Loops detectados: High-execution co-op", html)
    ...
    def test_generate_share_html_omits_personalized_behavioral_explanation_mismatch(self) -> None:
        html = generate_share_html(
            deals=[],
            vanity="gaben",
            min_discount=50,
    ...
    def test_generate_share_html_omits_invalid_personalized_behavioral_explanation(self) -> None:
        html = generate_share_html(
            deals=[],
            vanity="gaben",
            min_discount=50,
    ...
    def test_generate_html_budget_row_accepts_steam_appid_pick(self) -> None:
        html = generate_html(
            deals=[],
            backlog_on_sale=[],
            have_on_sale=[],
        ...
        self.assertIn("Steam AppID Original", html)
        self.assertIn('data-budget-row="balanced::30"', html)
        self.assertIn('data-budget-row-key="balanced::30"', html)
        self.assertIn("https://store.steampowered.com/app/30/", html)
        self.assertIn("steam/apps/30/capsule_231x87.jpg", html)
        self.assertIn("&quot;appid&quot;: &quot;30&quot;", html)
        self.assertNotIn("https://store.steampowered.com/app//", html)
    ...
    def test_generate_html_uses_first_budget_variant_when_selected_invalid(self) -> None:
        html = generate_html(
            deals=[],
            backlog_on_sale=[],
            have_on_sale=[],
        ...
        self.assertIn('class="budget-variant-panel hidden" data-budget-panel="large"', html)
        self.assertIn('Variante activa: <strong id="budget-current-variant">Lista chica</strong>', html)
        self.assertNotIn('data-budget-variant-btn="missing"', html)

    def test_generate_html_uses_root_budget_selection_when_selected_invalid(self) -> None:
        html = generate_html(
            deals=[],
            backlog_on_sale=[],
            have_on_sale=[],
        ...
        self.assertIn('data-budget-variant-btn="small"', html)
        self.assertIn('class="budget-variant-panel" data-budget-panel="small"', html)
        self.assertIn('data-budget-row="small::a"', html)
        self.assertIn("Alpha", html)
        self.assertIn('Variante activa: <strong id="budget-current-variant">Lista chica</strong>', html)
        panel_start = html.index('class="budget-variant-panel" data-budget-panel="small"')
        panel_end = html.index('class="budget-variant-panel hidden" data-budget-panel="large"')
        self.assertNotIn("budget-empty-state", html[panel_start:panel_end])
