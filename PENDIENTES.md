...
- [ ] [Track] Reducir el tiempo total de fetching en wishlists grandes. [Estado: ya hay `max_workers=16`, observabilidad de fallback individual, cooldown para fallos/no-data, parser offline de logs warm-cache, tabla comparativa de múltiples logs con deltas y bloque `Warm-cache next actions` con batch size sugerido/cooldown concreto; captura Windows muestra `HTTP 400` en batch y ETA muy alta como evidencia adicional. Falta: medir corrida larga real, ajustar cache/batches/invalidación por promos y reducir fallback individual lento. Evidencia: run grande con progreso estable, menor duración y artifacts completos.]
...
- [ ] [Quick Win] Cambiar el icono desktop/Windows para no mostrar el icono genérico de Python. [Estado: detectado en captura Windows del binario/ventana. Falta: definir/agregar icono propio y validar que el build Windows lo use. Evidencia: ejecutable/ventana Windows muestra icono de Steam Tools, no el default de Python.]
...
- [ ] [Quick Win] Probar el dashboard PAYDAY 2 con datos reales antes de expandir alcance. [Estado: parcial 2026-04-27: tests de assets/infra OK, dashboard local responde en `http://127.0.0.1:8081` con cache real, `/api/data` carga 80 DLCs y toggle reversible de DLC restaura conteos. Falta: validación visual/manual en browser, probar `Actualizar datos` si se quiere refrescar red y confirmar experiencia cómoda. Evidencia: sesión real sin errores HTTP visibles y marcado/desmarcado reversible documentado en `BITACORA.md`.]
