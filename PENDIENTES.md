...
- [ ] [Track] Dashboard histórico HTML. [Estado: MVP+ con cobertura automática reforzada para analytics/filtros/orden/parsing; captura Windows muestra `Historial por juego` con demasiados chips/ruido visual y ubicación discutible. Falta: validación manual final en browser, gráficos/tendencias más ricos, comparativa entre runs, deltas de wishlist y decidir si el historial por juego se colapsa, se limita con búsqueda/ver más o se mueve a una vista dedicada. Evidencia: navegación manual fluida entre runs y comparación clara en browser.]
