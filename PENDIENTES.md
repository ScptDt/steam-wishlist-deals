...
- [ ] [Track] Reforzar seguridad del server local y manejo de secretos. [Estado: auditoría whole-project detecta riesgos en endpoints locales: `/api/config` puede devolver secretos, subprocess recibe keys/tokens por argv, `/files/<name>` puede servir cualquier basename dentro del `output_dir`, HTML generado se sirve inline en el mismo origen y `steam_tools_desktop.py --run-script` acepta nombres de script sin allowlist estricta. Falta: redactar config pública, mover secretos fuera de argv, limitar `/files/...` a artefactos/extensiones esperadas, reducir riesgo same-origin de HTML inline, sanear errores expuestos y restringir `--run-script`. Evidencia: tests dirigidos para config redactada, path/file serving, argv/env de secretos, errores sin detalles internos y allowlist de scripts.]
- [ ] [Track] Reducir el tiempo total de fetching en wishlists grandes. [Estado: ya hay `max_workers=16`, observabilidad de fallback individual, cooldown para fallos/no-data, parser offline de logs warm-cache, tabla comparativa de múltiples logs con deltas y bloque `Warm-cache next actions` con batch size sugerido/cooldown concreto, cobertura de piso `batch_size=1`, menos recomendaciones genéricas duplicadas, circuit breaker conservador para evitar splits HTTP 400 repetidos antes de fallback individual y fixes UX de auth Steam 401/403 para que la CLI no caiga con traceback antes de llegar a performance. La auditoría warm-cache real confirma que cache/cooldown funciona, pero el costo principal sigue en cache frío/expirado, candidatos stale y fallback individual lento cuando Steam degrada batches con HTTP 400. Falta: medir una corrida grande con el circuit breaker nuevo, reducir costo de cold/stale refresh, ajustar batch/fallback y evaluar invalidación por promos solo después de controlar el fallback. Evidencia: 3 corridas reales BG00G: cache expirado `2,933` candidates en `7965.9s`, cache caliente parcial `436` candidates en `1107.3s`, cache caliente + cooldown `0` candidates en `1.4s`; intento nuevo BG00G sigue largo (`2,933` candidates, ETA ~80m) y se aborta manualmente; artifacts completos siguen pendientes del run desktop/binario.]
